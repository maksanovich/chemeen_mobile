...
export default function PRSPWScreen() {
    const router = useRouter();
    const dispatch = useDispatch();

    const { items } = useSelector((state) => state.master);

    const [PRSPWSStyles, setPRSPWSStyles] = useState<IMaster[]>([]);
    ...
    useEffect(() => {
        const result = getMasterListNames(items, 'PRSPWSStyle', 'PRSPWS');
        setPRSPWSStyles(result);
    }, [items])
    ...
    return (
        <ScrollView>
            <ThemedView style={styles.container}>
                ...
                {PRSPWSStyles.map((item: IMaster, index: number) => (
                    <MasterItem
                        key={index}
                        item={item}
                        removable={true}
