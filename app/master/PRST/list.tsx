...
export default function PRSTScreen() {
    const router = useRouter();
    const dispatch = useDispatch();

    const { items } = useSelector((state) => state.master);

    const [PRSTNames, setPRSTNames] = useState<IMaster[]>([]);
    ...
    useEffect(() => {
        const result = getMasterListNames(items, 'PRSTName', 'PRST');
        setPRSTNames(result);
    }, [items])

    const getPRST = async () => {
        try {
            const response = await axiosInstance.get('master/PRST');
    ...
    return (
        <ScrollView>
            <ThemedView style={styles.container}>
                ...
                {PRSTNames.map((item: IMaster, index: number) => (
                    <MasterItem
                        key={index}
                        item={item}
                        removable={true}
