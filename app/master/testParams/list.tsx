...
export default function TestParamsScreen() {
    const router = useRouter();
    const dispatch = useDispatch();

    const { items } = useSelector((state) => state.master);

    const [testParamsNames, setTestParamsNames] = useState<IMaster[]>([]);
    ...
    useEffect(() => {
        const result = getMasterListNames(items, 'testParams', 'testParams');
        setTestParamsNames(result);
    }, [items])

    const getTestParams = async () => {
        try {
            const response = await axiosInstance.get('master/testParams');
    ...
    const onPressAdd = () => {
        router.push('/master/testParams/add');
    }
    ...
    return (
        <ScrollView>
            <ThemedView style={styles.container}>
                ...
                {testParamsNames.map((item: IMaster, index: number) => (
                    <MasterItem
                        key={index}
                        item={item}
                        removable={true}
